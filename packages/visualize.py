...
# reusable bar graph plotter
def horizontal_bar_graph(xs, ys, plot_labels, size=(2, 2), fig_caption=None):
    # get x-axis
...
def plot_multiple(pairs: list, title: str) -> matplotlib.pyplot:
    """Plot multiple lines in Matplotlib."""
    # get initial coords
    xs, ys = pairs.pop()
    
    # plot initial line
    graph = line_graph(xs, ys, title)
    
    # now add each remaining coordinate
    for xs, ys in pairs:
        # create a line chart, years on x-axis, gdp on y-axis
        graph.plot(xs, ys, linestyle='solid')

    # returning pyplot object
    return graph